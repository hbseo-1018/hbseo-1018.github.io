...
if (work01) {
    ...
    work01.addEventListener('mouseleave', function () {
        work_a.reverse();
    });
}

if (work02) {
    work02.addEventListener('mouseover', function () {
        work_b.play();
    });
    work02.addEventListener('mouseleave', function () {
        work_b.reverse();
    });
}

if (work03) {
    work03.addEventListener('mouseover', function () {
        work_c.play();
    });
    work03.addEventListener('mouseleave', function () {
        work_c.reverse();
    });
}

const text = "Portfolio of HyeBin";
let index = 0;
let speed = 120;

function typeWriter() {
    const target = document.querySelector('.index_txt');
    if (!target) return;
...
function getModalPaths(fileName) {
    return [
        `./layers/${fileName}`,
        `../layers/${fileName}`,
        `/layers/${fileName}`
...
async function fetchModalContent(fileName) {
    const paths = getModalPaths(fileName);
